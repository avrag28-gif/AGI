-- FlightSim TCAS/ND traffic presentation v0.1
...
local rangeLabel=Instance.new("TextLabel"); rangeLabel.Position=UDim2.fromOffset(8,24); rangeLabel.Size=UDim2.new(1,-16,0,18); rangeLabel.BackgroundTransparency=1; rangeLabel.Text="RANGE 5 KM"; rangeLabel.Font=Enum.Font.Code; rangeLabel.TextSize=11; rangeLabel.Parent=root
...
local function addTarget(t)
 ...
 local radius=math.clamp(range/5000,0,1)*92; local rad=math.rad(rb); local px=.5+math.sin(rad)*radius/200; local py=.5-math.cos(rad)*radius/200
...
telemetry.OnClientEvent:Connect(function(state)
 ...
 rangeLabel.Text=string.format("RANGE 5 KM   %s",td.Level or "NONE")
