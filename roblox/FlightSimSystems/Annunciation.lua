-- FlightSim cockpit annunciation / warning logic v0.2
-- Presentation-neutral warning derivation from authoritative aircraft state.
...
function Annunciation:Step(dt)
 ...
 local airborne=x.GroundContact~=true
 local speed=math.max(tonumber(x.Airspeed) or 0,0); local altitude=math.max(tonumber(x.Altitude) or 0,0)
 ...
 local gearUnsafe=airborne and speed>120 and gear.DownLocked~=true and altitude<3000
 local flapOver=(x.FlapSystem and x.FlapSystem.OverSpeed)==true
 local hydraulicLow=airborne and math.max(tonumber(hyd.A) or 0,tonumber(hyd.B) or 0)<900
 local electricalLoss=airborne and not (elec.Bus1==true or elec.Bus2==true)
 local lowFuel=(tonumber(fuel.Total) or 0)<3000
 local engineOut=airborne and speed>80 and ((engines[1] and engines[1].Running==false) or (engines[2] and engines[2].Running==false))
 if fire then table.insert(msgs,"FIRE") end
 if stall then table.insert(msgs,"STALL") end
 if gearUnsafe then table.insert(msgs,"GEAR UNSAFE") end
 if flapOver then table.insert(msgs,"FLAP OVERSPEED") end
 if hydraulicLow then table.insert(msgs,"HYDRAULIC LOW") end
 if electricalLoss then table.insert(msgs,"ELECTRICAL") end
 if lowFuel then table.insert(msgs,"LOW FUEL") end
 if engineOut then table.insert(msgs,"ENGINE OUT") end
