-- FlightSim aerodynamic flight dynamics foundation v0.8
...
function Physics:Step(dt)
 ...
 -- Normalized flap command is treated as a continuous aerodynamic schedule.
 -- The nonlinear lift/drag terms approximate the stronger effect at landing flap.
 local flapLift=0.52*flap+0.20*flap*flap
 local flapDrag=0.025*flap+0.035*flap*flap
 local cl=0.45+0.085*aoa+flapLift
 local stallAoA=15
 local stallRatio=clamp((math.abs(aoa)-stallAoA)/8,0,1)
 cl*=1-0.75*stallRatio
 local cd0=0.028+flapDrag+(gearDown and 0.02 or 0)
 ...
 -- Approximate 737 rotation behavior: strong nose-up authority becomes useful
 -- near rotation speed instead of allowing a large pitch change at taxi speed.
 if x.GroundContact then
  local rotationFactor=clamp((speedKts-65)/25,0,1)
  elevator*=0.25+0.75*rotationFactor
 end
 ...
 -- Compute the aerodynamic vertical force before ground constraints.
 local freeVerticalAccel=(lift-weight*math.cos(math.rad(x.Roll)))/math.max(mass,1)
 local freeVerticalSpeed=(x.VerticalSpeed or 0)+freeVerticalAccel*dt
 freeVerticalSpeed*=clamp(1-0.08*math.abs(x.Roll)/45,0.6,1)

 -- Lift-off requires both adequate airspeed and positive vertical force. This
 -- prevents the aircraft from leaving the runway because of a single frame.
 local liftRatio=lift/math.max(weight,1)
 local canLiftOff=x.GroundContact and speedKts>=90 and liftRatio>=0.92 and freeVerticalSpeed>0.5
 if canLiftOff then
  x.GroundContact=false
  x.VerticalSpeed=freeVerticalSpeed
 else
  x.VerticalSpeed=clamp(freeVerticalSpeed,-80,80)
  if x.GroundContact then x.VerticalSpeed=0 end
 end
