-- FlightSim aerodynamic / ground dynamics foundation v1.9
...
function Physics:Step(dt)
 ...
 local effectiveAirspeed=clamp(tonumber(weather.EffectiveAirspeed) or speedKts,0,500)
 local aeroSpeedMS=effectiveAirspeed*0.514444
 local icingDragFactor=clamp(tonumber(weather.IcingDragFactor) or 1,1,1.12)
 local icingLiftFactor=clamp(tonumber(weather.IcingLiftFactor) or 1,0.92,1)
 local turbulencePitch=clamp(tonumber(weather.TurbulencePitch) or 0,-1.5,1.5)
 local turbulenceRoll=clamp(tonumber(weather.TurbulenceRoll) or 0,-1.5,1.5)
 local crosswind=clamp(tonumber(weather.CrosswindKts) or 0,-60,60)
 ...
 local cl=(0.45+0.085*aoa+flapLift)*icingLiftFactor; local stallAoA=15; local stallRatio=clamp((math.abs(aoa)-stallAoA)/8,0,1); cl*=1-0.75*stallRatio
 local cd0=0.028+flapDrag+0.02*gearExposed; local cd=(cd0+0.045*cl*cl+0.08*stallRatio)*icingDragFactor
 ...
 local qFactor=clamp(effectiveAirspeed/140,0.12,1.2); local hydraulicAuthority=clamp(math.max(x.Hydraulic.A or 0,x.Hydraulic.B or 0)/1800,0,1)
 ...
 if ground then
  local groundYaw=(x.GroundSteering and x.GroundSteering.YawRate) or 0; yawRate=groundYaw; rollRate=approach(rollRate,rollControl+0.05*turbulenceRoll,8,dt); pitchRate=approach(pitchRate,pitchControl-staticPitchStability+0.05*turbulencePitch,7,dt); x.Sideslip=approach(x.Sideslip or 0,clamp(crosswind*0.03,-3,3),4,dt)
 else
  ...
  local windBeta=clamp(crosswind/math.max(effectiveAirspeed,60)*57.2958,-5,5)
  local desiredBeta=clamp(rudder*4.5+turnRate*0.10-rollControl*0.035+windBeta*0.12,-10,10); local beta=x.Sideslip or x.Beta or 0; local betaRate=(desiredBeta-beta)*2.8-beta*0.55-yawRate*0.045; beta=clamp(beta+betaRate*dt,-12,12)
  local weathercock=-beta*0.95; local yawDamping=-yawRate*0.72; local adverseYaw=-aileron*1.4*qFactor*hydraulicAuthority; local targetYawRate=turnRate*0.32+rudderControl+weathercock+yawDamping+adverseYaw+engineYawMoment+clamp(crosswind*0.015,-0.9,0.9)
  yawRate=approach(yawRate,targetYawRate,5.5,dt)
  local rollDamping=-rollRate*0.58*qFactor; rollRate=approach(rollRate,rollControl+rollDamping+0.25*turbulenceRoll,7.0,dt); local pitchDamping=-pitchRate*0.35*qFactor; local longitudinalMoment=pitchControl-staticPitchStability-speedStability+pitchDamping+0.25*turbulencePitch; pitchRate=approach(pitchRate,longitudinalMoment,5.5,dt); x.Sideslip=beta; x.Beta=beta; x.TurnCoordination=clamp(1-math.abs(beta)/6,0,1)
 end
 ...
 local flightPathAngle=math.rad((x.Pitch or 0)-aoa); local energyVertical=math.sin(flightPathAngle)*aeroSpeedMS; local freeVerticalAccel=(lift-weight*math.cos(math.rad(x.Roll or 0)))/math.max(mass,1); local freeVerticalSpeed=(x.VerticalSpeed or 0)+freeVerticalAccel*dt
 ...
 local headingRad=math.rad(x.Heading or 0); local forward=CFrame.Angles(math.rad(-(x.Pitch or 0)),math.rad(x.Heading or 0),0).LookVector
 local windU=tonumber(weather.WindUKts) or 0; local windV=tonumber(weather.WindVKts) or 0
 local windWorld=Vector3.new(windU*0.514444,0,windV*0.514444)
 x.Velocity=forward*(x.Airspeed*0.514444)+windWorld
