-- FlightSim aerodynamic / ground dynamics foundation v2.4
...
function Physics:Step(dt)
 ...
 local rho=1.225*math.exp(-math.max(finite(x.Altitude,0),0)/8500); local wingArea=125; local trim=clamp(finite(x.TrimPitch,0),-8,8)
 -- Keep angle-of-attack as a flight-state quantity instead of mirroring pitch. This preserves a
 -- meaningful pitch-versus-flight-path relationship so a go-around pitch command produces climb.
 local pitchAngle=finite(x.Pitch,0); local aoa=clamp(2.5+0.15*(pitchAngle-trim)+0.20*flap,-20,30)
 ...
 local fpa=math.rad(x.Pitch-aoa); local targetVSMS=math.sin(fpa)*aeroSpeedMS; local vsFpm=finite(x.VerticalSpeed,0); local vsMS=vsFpm*FPM_TO_MS; local vAccel=(lift-weight*math.cos(math.rad(x.Roll)))/math.max(mass,1); vsMS+=vAccel*dt; if not ground then vsMS=approach(vsMS,targetVSMS,18,dt) end; vsMS*=clamp(1-.08*math.abs(x.Roll)/45,.6,1)
 local liftoff=ground and speedKts>=90 and lift/math.max(weight,1)>=.92 and vsMS>0.5
 if liftoff then x.GroundContact=false else if ground then vsMS=0 end end
