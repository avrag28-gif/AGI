-- FlightSim autopilot / ILS capture / go-around foundation v0.7
...
function Autopilot:Step(dt)
 local x=self.state:Get(); local ap=x.Autopilot or {}; local nav=x.Navigation or {}; local v=x.VNAV or {}; local ils=nav.ILS
 local altitude=finite(x.Altitude) and x.Altitude or 0
 local heading=finite(x.Heading) and x.Heading%360 or 0
 ...
 if not ap.Enabled then
  ...
  ap.Mode="OFF"; return true
 end
 if ap.GoAround then
  nav.Mode="HDG"; v.Mode="OFF"
  if ils then ils.LocalizerCaptured=false; ils.GlideSlopeCaptured=false end
  ap.ILSLocalizerCaptured=false; ap.ILSGlideSlopeCaptured=false; ap.Mode="GO_AROUND"
  local targetH=finite(ap.GoAroundHeading) and ap.GoAroundHeading%360 or heading
  local baseAltitude=finite(ap.GoAroundAltitude) and ap.GoAroundAltitude or altitude
  local targetA=math.max(baseAltitude,altitude+1000)
  ap.TargetHeading=targetH; ap.TargetAltitude=targetA
  local he=err(targetH,heading); local ae=targetA-altitude
  self.bankCommand=slew(self.bankCommand,clamp(he/30,-0.65,0.65),1.8,dt)
  self.pitchCommand=slew(self.pitchCommand,clamp(ae/1200,-0.40,0.40),1.5,dt)
  ap.CommandBank=self.bankCommand; ap.CommandPitch=self.pitchCommand; ap.CommandAileron=self.bankCommand; ap.CommandElevator=self.pitchCommand
  return true
 end
 ...
 if mode=="APP" and ils and ils.LocalizerValid then
  ...
  ap.ILSGlideSlopeCaptured=ils.GlideSlopeCaptured==true and ap.ILSLocalizerCaptured
  targetH=firstFinite(nav.CommandHeading,heading)%360
 ...
 local he=err(targetH,heading); local altitudeError=targetA-altitude
 local app=mode=="APP" and ils and ils.LocalizerValid
 local bankLimit=app and 0.75 or 0.65
 local bankGain=app and 1/12 or 1/30
 local targetBank=clamp(he*bankGain,-bankLimit,bankLimit)
 local pitchGain=(app and ils.GlideSlopeCaptured) and 1/500 or 1/1200
 local pitchLimit=(app and ils.GlideSlopeCaptured) and 0.32 or 0.45
 ...
 elseif ap.Mode=="ALT_HOLD" and math.abs(altitudeError)<40 then targetPitch=clamp(-(tonumber(x.VerticalSpeed) or 0)/1800,-0.20,0.20) end
