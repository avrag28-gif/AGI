-- FlightSim failure-system integration contract tests v0.2
...
local function run()
 ...
 check(failures:SetAPUFire(true),"APU fire setter failed"); failures:Step(1); fire:Step(1); check(s.data.FireProtection.APU.Fire,"APU fire must be annunciated"); check(not s.data.APU.Running and not s.data.APU.GeneratorAvailable,"APU fire must shut down the APU")
 check(failures:SetEngine(1,false),"engine clear failed"); check(failures:SetElectrical("Bus1",false),"Bus1 clear failed"); check(failures:SetHydraulic("A",false),"Hydraulic A clear failed"); check(failures:SetFlightControl("Aileron",false),"Aileron clear failed"); check(failures:SetPressurization("Pack1",false),"Pack1 clear failed"); check(failures:SetPressurization("OutflowValve",false),"outflow-valve clear failed"); check(failures:SetAntiIce("Engine2",false),"anti-ice clear failed"); failures:Step(1); check(not s.data.FailureEffects.Engine1Failed and not s.data.FailureEffects.ElectricalBus1Failed and not s.data.FailureEffects.HydraulicAFailed and s.data.FailureEffects.AileronAuthority==1 and not s.data.FailureEffects.Pack1Failed and not s.data.FailureEffects.OutflowValveFailed and not s.data.FailureEffects.AntiIceEngine2Failed,"cleared failure effects are stale")
 return true
end
return {Run=run}
