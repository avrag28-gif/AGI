-- FlightSim cabin pressurization system v0.4
-- Game-simulation model; values are tunable and are not certified aircraft data.
local Pressurization={}; Pressurization.__index=Pressurization
local function clamp(v,a,b) return math.max(a,math.min(b,v)) end
local function finite(v) return type(v)=="number" and v==v and v>-math.huge and v<math.huge end
local function ensure(x)
 x.Pressurization=x.Pressurization or {}; local p=x.Pressurization
...
function Pressurization:Step(dt)
 ...
 -- Maintain a nominal cabin altitude below 8,000 ft in normal flight, with a controlled approach to the landing altitude during descent.
 local targetCabin=math.min(8000,aircraftAlt)
 if p.LandingAltitudeFt>0 and aircraftAlt<12000 then targetCabin=math.min(targetCabin,math.max(0,p.LandingAltitudeFt+500)) end
 if p.Dump then targetCabin=aircraftAlt end
 local desiredValve
 if p.Auto then
  local cabinError=targetCabin-p.CabinAltitudeFt
  desiredValve=clamp(0.28+0.000045*cabinError+(p.SourceAvailable and 0 or 0.38),0.08,1)
 else desiredValve=p.ManualCommand end
 ...
 local leakRate=25+90*p.OutflowValve; local sourceRate=p.SourceAvailable and (65+35*clamp(tonumber(bleed.TotalAirflow) or 0,0,2)/2) or 0
 local rate=clamp((targetCabin-p.CabinAltitudeFt)*0.06+leakRate-sourceRate,-1200,1200)
 if not p.SourceAvailable then rate=math.max(rate,180) end
