-- FlightSim cabin pressurization system v0.2
...
local function ensure(x)
 ...
 local p=x.Pressurization; p.Auto=p.Auto~=false; p.OutflowValve=clamp(tonumber(p.OutflowValve) or 0.35,0,1); p.ManualCommand=clamp(tonumber(p.ManualCommand) or p.OutflowValve,0,1); return p
end
...
function Pressurization:Step(dt)
 local x=self.state:Get(); local p=ensure(x); dt=math.max(0,tonumber(dt) or 0); local bleed=x.BleedAir or {}; local aircraftAlt=math.max(0,tonumber(x.Altitude) or 0)
 ...
 p.Warning=p.CabinAltitudeWarning or p.DifferentialWarning or (not p.SourceAvailable and aircraftAlt>5000); x.Pressurization=p
end
