...
function Test.Run()
 ...
 local rudderLowSpeed=newState(); rudderLowSpeed.Airspeed=135; rudderLowSpeed.Controls.Rudder=1; FlightControls.new(rudderLowSpeed):Step(0.1)
 local rudderHighSpeed=newState(); rudderHighSpeed.Airspeed=340; rudderHighSpeed.Controls.Rudder=1; FlightControls.new(rudderHighSpeed):Step(0.1)
 check(rudderHighSpeed.ControlFeel.RudderBlowdownFactor<rudderLowSpeed.ControlFeel.RudderBlowdownFactor,"rudder blowdown did not reduce high-speed rudder authority")
 check(math.abs(rudderHighSpeed.Surface.Rudder)<math.abs(rudderLowSpeed.Surface.Rudder),"high-speed rudder surface travel did not reduce under blowdown")
 local engineOutRudder=newState(); engineOutRudder.Airspeed=340; engineOutRudder.Controls.Rudder=1; engineOutRudder.Engines[1].Thrust=120000; engineOutRudder.Engines[2].Thrust=0; FlightControls.new(engineOutRudder):Step(0.1)
 check(engineOutRudder.ControlFeel.RudderBlowdownFactor>rudderHighSpeed.ControlFeel.RudderBlowdownFactor,"engine-out asymmetric thrust did not preserve additional rudder authority")
