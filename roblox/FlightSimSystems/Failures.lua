-- FlightSim failure manager foundation v0.5
...
local function ensure(x)
 ...
 for i=1,2 do x.Failures.Engines[i].Fire=x.Failures.Engines[i].Fire==true; x.Failures.Engines[i].Active=x.Failures.Engines[i].Active==true end
 x.Failures.Hydraulic=x.Failures.Hydraulic or {A=false,B=false}; x.Failures.Electrical=x.Failures.Electrical or {Bus1=false,Bus2=false,APU=false}; x.Failures.FlightControls=x.Failures.FlightControls or {Aileron=false,Elevator=false,Rudder=false}; x.Failures.FireProtection=x.Failures.FireProtection or {Engines={[1]={Fire=false},[2]={Fire=false}},APU={Fire=false}}
...
function Failures:Step(dt)
 local x=self.state:Get(); local f=ensure(x); x.FailureEffects=x.FailureEffects or {AileronAuthority=1,ElevatorAuthority=1,RudderAuthority=1}
 for i=1,2 do local e=x.Engines[i]; local ef=f.Engines[i]; if ef.Active then e.FuelOn=false; e.Ignition=false; e.Starter=false; e.Running=false; e.Thrust=0; e.FuelFlow=0; e.GeneratorAvailable=false end end
 if f.Hydraulic.A then x.Hydraulic.A=0 end; if f.Hydraulic.B then x.Hydraulic.B=0 end; if f.Electrical.Bus1 then x.Electrical.Bus1=false end; if f.Electrical.Bus2 then x.Electrical.Bus2=false end; if f.Electrical.APU then x.Electrical.APU=false end
