...
local function ensure(x)
 ...
 f.APU=f.APU or {Fire=false,Overheat=false,Warning=false,Detector=0,Armed=true}
 f.FireTest=f.FireTest==true
 return f
end
function FireProtection.new(state) return setmetatable({state=state},FireProtection) end
function FireProtection:Step(dt)
 local x=self.state:Get(); local f=ensure(x); local failures=x.Failures and x.Failures.FireProtection or {}
 for i=1,2 do
