...
function Fuel:_feedEngine(tanks,s,index,demand)
 ...
 if source=="LEFT" then take("Left","LEFT")
 elseif source=="RIGHT" then take("Right","RIGHT")
 elseif source=="CENTER" then take("Center","CENTER")
 ...
 local label=#sources>0 and table.concat(sources,"+") or "NONE"
 return used,label
end
function Fuel:Step(dt)
 ...
 if d1>0 and used1<=0 and x.Engines[1] then x.Engines[1].FuelOn=false end
 if d2>0 and used2<=0 and x.Engines[2] then x.Engines[2].FuelOn=false end
