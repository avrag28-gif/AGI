...
local function pressure(system) return type(system)=="table" and (tonumber(system.Pressure) or 0) or (tonumber(system) or 0) end
...
function FlightControls:Step(dt)
 ...
 local speed=math.max(tonumber(x.Airspeed) or 0,0); local hydraulic=clamp(math.max(pressure(h.A),pressure(h.B),pressure(h.Standby))*0.000555556,0,1)
