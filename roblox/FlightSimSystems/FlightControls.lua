...
function FlightControls:Step(dt)
 ...
 -- The 737 rudder has air-load/blowdown behavior: available powered travel
 -- decreases as dynamic pressure rises. This is a simulation approximation;
 -- it is intentionally not presented as an exact NG rudder-angle schedule.
 local engineThrust1=math.max(tonumber((x.Engines or {})[1] and (x.Engines or {})[1].Thrust) or 0,0)
 local engineThrust2=math.max(tonumber((x.Engines or {})[2] and (x.Engines or {})[2].Thrust) or 0,0)
 local totalEngineThrust=engineThrust1+engineThrust2
 local thrustAsymmetry=totalEngineThrust>1 and math.abs(engineThrust2-engineThrust1)/totalEngineThrust or 0
 local vmo=340
 local blowdown=1
 if not ground then
  local speedFraction=clamp((speed-135)/math.max(vmo-135,1),0,1)
  blowdown=clamp(1-0.45*speedFraction,0.55,1)
  -- High asymmetric thrust is a condition where the rudder must retain more
  -- available authority for directional control.
  blowdown=clamp(blowdown+0.18*thrustAsymmetry,0.55,1)
 end
 ...
 local ailTarget=ail*ailHyd*groundAileron*ailFailure; local eleTarget=ele*eleHyd*groundElevator*eleFailure; local rudTarget=rudderCommand*rudHyd*groundRudder*rudFailure*blowdown
 ...
 x.ControlFeel.AileronAuthority=clamp(ailHyd*groundAileron*ailFailure,0,1); x.ControlFeel.ElevatorAuthority=clamp(eleHyd*groundElevator*eleFailure,0,1); x.ControlFeel.RudderAuthority=clamp(rudHyd*groundRudder*rudFailure*blowdown,0,1); x.ControlFeel.RudderBlowdownFactor=blowdown
 ...
 local ailLoad=math.abs(ail)*groundAileron*ailFailure; local eleLoad=math.abs(ele)*groundElevator*eleFailure; local rudLoad=math.abs(rudderCommand)*groundRudder*rudFailure*blowdown
