-- FlightSim landing gear / brake contract tests v0.1
...
local function run()
 ...
 gear:Step(1); check(s.data.GearStatus.DownLocked,"hydraulic gear extension must reach down lock")
 s.data.Gear={Nose=false,Left=false,Right=false}; gear:Step(1); check(s.data.GearStatus.UpLocked,"hydraulic gear retraction must reach up lock")
 s.data.Hydraulic.A=0; s.data.Hydraulic.B=0; s.data.Gear={Nose=true,Left=true,Right=true}; gear:Step(1); check(s.data.GearStatus.Transitioning==false,"failed dual hydraulics must not move gear")
 ...
 s.data.Hydraulic.A=0; s.data.Hydraulic.B=0; s.data.Brakes.Parking=false; s.data.Brakes.ToeBrake=1; brakes:Step(1); check(s.data.Brakes.BrakePressure==0,"brakes must release when hydraulic pressure is unavailable")
 return true
end
return {Run=run}
