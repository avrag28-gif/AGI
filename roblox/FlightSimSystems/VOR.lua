-- FlightSim VOR signal computation v0.6
-- Produces raw NAV1/NAV2 VOR candidates. NAVReceiver owns receiver selection.
...
local function solve(n,x,station,index,course)
 ...
 if not station or not freqMatch(frequency,station.Frequency) then n[p.."VORSignal"]=nil; return nil end
 local distance=dist(x.Position,station.Position); local toStation=bearing(x.Position,station.Position); local radial=wrap(toStation+180); local selected=wrap(tonumber(course) or radial); local courseError=err(selected,radial); local angle=math.abs(err(selected,toStation)); local to=angle<=90; local dme=station.DME==true
 local signal={Available=true,Source="VOR",Ident=station.Ident,Frequency=station.Frequency,Distance=distance,DMEDistanceNM=dme and distance/1852 or nil,DMEAvailable=dme,BearingToStation=toStation,Radial=radial,Course=selected,CourseError=courseError,TO=to,FROM=not to}
 n[p.."VORSignal"]=signal; return signal
end
...
function VOR:Step(dt)
 ...
 n.VOR=solve(n,x,n.VORStation,1,n.VORCourse)
