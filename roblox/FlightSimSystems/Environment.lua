-- FlightSim environment / weather foundation v0.1
-- Deterministic local atmosphere model with externally settable weather values.
local Environment={}; Environment.__index=Environment
local function clamp(v,a,b) return math.max(a,math.min(b,v)) end
local function finite(v) return type(v)=="number" and v==v and v>-math.huge and v<math.huge end
local function ensure(x)
 x.Environment=x.Environment or {WindDirection=0,WindSpeed=0,TemperatureC=15,PressureHpa=1013.25,VisibilityKm=50,Precipitation=0,Turbulence=0,Icing=0,CloudBaseFt=12000,CeilingFt=25000,Thunderstorm=false,OutsideAirTemperatureC=15,WindU=0,WindV=0,WindW=0}
...
function Environment:Step(dt)
 local x=self.state:Get(); local e=ensure(x); local altitude=math.max(0,tonumber(x.Altitude) or 0); local lapse=clamp(15-0.0065*altitude,-60,15); e.OutsideAirTemperatureC=e.TemperatureC + (lapse-e.TemperatureC)*clamp((altitude/20000),0,1)
 local rad=math.rad(e.WindDirection); e.WindU=-math.sin(rad)*e.WindSpeed; e.WindV=-math.cos(rad)*e.WindSpeed; e.WindW=e.Turbulence*(math.sin(os.clock()*1.7)*2)
 x.Weather={WindDirection=e.WindDirection,WindSpeed=e.WindSpeed,TemperatureC=e.OutsideAirTemperatureC,PressureHpa=e.PressureHpa,VisibilityKm=e.VisibilityKm,Precipitation=e.Precipitation,Turbulence=e.Turbulence,Icing=e.Icing,Thunderstorm=e.Thunderstorm}
end
return Environment
