-- FlightSim nose-wheel steering / taxi dynamics v0.1
-- Server-authoritative steering foundation. Values are simulation approximations, not certified aircraft data.
...
function GroundSteering:Step(dt)
 ...
 local available=(x.GroundContact==true) and g.DownLocked==true and hydraulic>=900
 ...
 if available and speed>1 then
  yawRate=clamp((angle/maxAngle)*28*steeringAuthority,-18,18)
 end
 ...
 gs.DifferentialBrakeAssist=0
 if available and speed>2 and speed<18 then
  local left=(x.Brakes and x.Brakes.LeftPressure) or 0
  local right=(x.Brakes and x.Brakes.RightPressure) or 0
  gs.DifferentialBrakeAssist=clamp((right-left)*8,-8,8)
  yawRate=clamp(yawRate+gs.DifferentialBrakeAssist,-18,18)
  gs.YawRate=yawRate
 end
 x.GroundSteering=gs
end
