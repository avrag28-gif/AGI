-- FlightSim electrical subsystem contract-test helpers v0.2
...
function ElectricalTest.ValidateBuses(state)
 ...
 assertBool(e.Bus1,"electrical Bus1 must be boolean"); assertBool(e.Bus2,"electrical Bus2 must be boolean")
...
function ElectricalTest.ValidateFailureIsolation(state)
 ...
 if f.APU==true then assert((x.ElectricalState or {}).APUGenerator~=true,"failed APU generator must not be available") end
 return true
end
function ElectricalTest.ValidateBattery(state)
 local x=state:Get(); local e=x.Electrical or {}; local charge=tonumber(x.BatteryCharge) or 0
 ...
 if (x.ElectricalState or {}).BatteryCharge~=nil then assert(math.abs((x.ElectricalState.BatteryCharge or 0)-charge)<0.0001,"battery telemetry mismatch") end
 return true
end
function ElectricalTest.ValidateSources(state)
 local x=state:Get(); local e=x.Electrical or {}; local es=x.ElectricalState or {}
 assert(type(es.Source1)=="string" and type(es.Source2)=="string","electrical source telemetry missing")
 assertBool(es.APUGenerator,"APU generator telemetry missing"); assertBool(es.Generator1,"GEN1 telemetry missing"); assertBool(es.Generator2,"GEN2 telemetry missing")
 assertBool(es.ExternalPower,"external power telemetry missing"); assertBool(es.BatteryConnected,"battery telemetry missing")
...
function ElectricalTest.ValidateAll(state)
 ElectricalTest.ValidateBuses(state); ElectricalTest.ValidateFailureIsolation(state); ElectricalTest.ValidateBattery(state); ElectricalTest.ValidateSources(state)
 return true
end
