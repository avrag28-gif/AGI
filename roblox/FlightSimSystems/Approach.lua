...
function Approach:Step(dt)
 ...
 if not r then n.ILS=nil; self.locCaptured=false; self.gsCaptured=false; return true end
 ...
 if not ilsAvailable then
  ...
  self.locCaptured=false; self.gsCaptured=false; return true
 end
 ...
 n.ILS={Available=true,Localizer=localizer,GlideSlope=gsError,GlideSlopeError=gsError,Distance=horizontal,Bearing=wrap(math.deg(math.atan2(r.Position.X-x.Position.X,r.Position.Z-x.Position.Z))),CourseError=hdgErr(r.Heading,x.Heading),DesiredAltitude=desiredAlt,LocalizerValid=locValid,GlideSlopeValid=gsValid,LocalizerCaptured=self.locCaptured,GlideSlopeCaptured=self.gsCaptured,FrontCourse=front,Mode="ARMED",TunedFrequency=tuned,RequiredFrequency=r.ILSFrequency,Ident=r.ILSIdent}
 if n.Mode=="APP" and locValid then n.ILS.Mode=self.locCaptured and (self.gsCaptured and "GS_CAPTURE" or "LOC_CAPTURE") or "CAPTURE" end
 return true
end
return Approach
