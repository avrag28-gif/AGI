...
function Brakes:Step(dt)
 ...
 local pressureA=pressure(h.A); local pressureB=pressure(h.B); local hydraulic=math.max(pressureA,pressureB); local available=clamp(hydraulic/1800,0,1)
 local toe=clamp(tonumber(b.ToeBrake) or 0,0,1); local parking=b.Parking==true and 1 or 0; local commanded=math.max(toe,parking)
 -- Demand represents what the pilot is requesting, not what hydraulic pressure can currently deliver.
 -- This prevents a pressure -> delivered pressure -> demand feedback loop that could falsely restore a failed system.
