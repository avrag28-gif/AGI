-- FlightSim ATC traffic / runway occupancy foundation v0.1
...
function ATCTraffic.new()
	return setmetatable({runways={},sequence=0},ATCTraffic)
end

function ATCTraffic:RegisterRunway(runwayId)
	local id=validKey(runwayId); if not id then return false,"invalid_runway" end
	if not self.runways[id] then
		self.runways[id]={Id=id,State="VACANT",AircraftId=nil,Callsign=nil,Operation=nil,Sequence=0}
	end
	return true,self.runways[id]
end

function ATCTraffic:GetRunway(runwayId)
	return self.runways[validKey(runwayId)]
end
...
function ATCTraffic:Reserve(runwayId,aircraftId,callsign,operation)
	...
	self.sequence+=1
	runway.State="RESERVED"; runway.AircraftId=aid; runway.Callsign=validKey(callsign) or aid; runway.Operation=op; runway.Sequence=self.sequence
	return true,runway
end
...
function ATCTraffic:SetOccupied(runwayId,aircraftId)
	local runway=self:GetRunway(runwayId); if not runway then return false,"runway_not_registered" end
	local aid=validKey(aircraftId); if not aid then return false,"invalid_aircraft_id" end
	if runway.State~="RESERVED" or runway.AircraftId~=aid then return false,"runway_not_reserved_by_aircraft" end
	runway.State="OCCUPIED"
	return true,runway
end
...
function ATCTraffic:SetHoldShort(runwayId,aircraftId,callsign)
	...
	if runway.State=="RESERVED" and runway.AircraftId~=aid then return false,"runway_reserved" end
	self.sequence+=1
	runway.State="HOLD_SHORT"; runway.AircraftId=aid; runway.Callsign=validKey(callsign) or aid; runway.Operation="CROSSING"; runway.Sequence=self.sequence
	return true,runway
end
...
function ATCTraffic:Release(runwayId,aircraftId)
	...
	runway.State="VACANT"; runway.AircraftId=nil; runway.Callsign=nil; runway.Operation=nil; runway.Sequence=0
	return true,runway
end
...
function ATCTraffic:ForEach(callback)
	for id,runway in pairs(self.runways) do callback(id,runway) end
end
