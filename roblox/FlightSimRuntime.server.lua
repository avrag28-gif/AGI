-- FlightSim modular runtime v0.6
...
local simulations={}
local function getFMC(id) local s=simulations[id]; return s and s.fmc end
local router=CommandRouter.new(registry,getFMC)
...
Players.PlayerAdded:Connect(createAircraftForPlayer); Players.PlayerRemoving:Connect(removeAircraftForPlayer)
...
local function simulationStep(dt)
	registry:ForEach(function(id)
		local sim=simulations[id]; if not sim then return end
		sim.electrical:Step(dt); sim.engine:Step(dt); sim.electrical:Step(dt); sim.core:Step(dt)
		sim.fmc:Step(dt); sim.waypoints:Step(dt); sim.navigation:Step(dt); sim.vnav:Step(dt); sim.autopilot:Step(dt)
		sim.flightControls:Step(dt); sim.landingGear:Step(dt); sim.brakes:Step(dt); sim.avionics:Step(dt); sim.physics:Step(dt)
	end)
end
RunService.Heartbeat:Connect(function(frameDt)
	...
	if telemetryAccumulator>=1/(tonumber(Config.TelemetryRate) or 20) then
		telemetryAccumulator=0; local telemetry=remotes:FindFirstChild("Telemetry")
		if telemetry then registry:ForEach(function(_,state,owner) if owner then telemetry:FireClient(owner,state:Get()) end end) end
	end
end)
