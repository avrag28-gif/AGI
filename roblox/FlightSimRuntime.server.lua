-- FlightSim modular runtime v0.7
...
local Navigation=require(systemsFolder:WaitForChild("Navigation")); local WaypointManager=require(systemsFolder:WaitForChild("WaypointManager")); local Autopilot=require(systemsFolder:WaitForChild("Autopilot")); local VNAV=require(systemsFolder:WaitForChild("VNAV")); local FMC=require(systemsFolder:WaitForChild("FMC")); local Physics=require(systemsFolder:WaitForChild("Physics"))
...
local router=CommandRouter.new(registry,function(id) return get(id,"fmc") end,function(id) return get(id,"radio") end,function(id) return get(id,"transponder") end)
...
local function simulationStep(dt)
	registry:ForEach(function(id)
		...
		s.fmc:Step(dt); s.waypoints:Step(dt); s.navigation:Step(dt); s.vnav:Step(dt); s.autopilot:Step(dt); s.flightControls:Step(dt); s.landingGear:Step(dt); s.brakes:Step(dt); s.avionics:Step(dt); s.radio:Step(dt); s.transponder:Step(dt); s.physics:Step(dt)
	end)
end
