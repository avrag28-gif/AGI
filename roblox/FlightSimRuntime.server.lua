-- FlightSim modular runtime v3.1
...
local router=CommandRouter.new(registry,function(id)return get(id,"fmc") end,function(id)return get(id,"radio") end,function(id)return get(id,"transponder") end,function(id)return get(id,"mcp") end,function(id)return get(id,"approach") end,function(id)return get(id,"autothrottle") end)
...
local function simulationStep(dt)
 registry:ForEach(function(id)
  ...
  s.fmc:Step(dt); s.mcp:Step(dt); s.approach:Step(dt); s.vor:Step(dt); s.navReceiver:Step(dt); s.navigation:Step(dt); s.vnav:Step(dt); s.autopilot:Step(dt); s.autothrottle:Step(dt); s.trim:Step(dt); s.flaps:Step(dt)
  s.flightControls:Step(dt); s.landingGear:Step(dt); s.brakes:Step(dt); s.hydraulic:Step(dt)
