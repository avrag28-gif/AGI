-- FlightSim modular runtime v2.7
...
local function simulationStep(dt)
 registry:ForEach(function(id)
  ...
  s.fmc:Step(dt); s.mcp:Step(dt); s.navigation:Step(dt); s.approach:Step(dt); s.vor:Step(dt); s.vnav:Step(dt); s.autopilot:Step(dt); s.autothrottle:Step(dt); s.trim:Step(dt); s.flaps:Step(dt)
